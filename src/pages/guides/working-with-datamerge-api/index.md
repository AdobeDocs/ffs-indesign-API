...
**Why:** Variable file naming gives each data row a single output file name via the `&` column, so it only applies when the merge job has a clear one row–to–one name mapping. If `allowMultipleRecordsPerPage` is true, several records can share a page, so row-level names are ambiguous. If `recordsPerFile` is not 1, records are merged into multi-page output documents (and for PDF and InDesign you typically get one file per range folder), so many rows contribute to the same file and per-row file names are no longer meaningful. In those situations, variable file naming is not available for the affected output types.
